...
RustVault offers something they cannot: **Total Isolation.**
...
2. **Micro-Footprint Architecture:** Unlike Bitwarden (which requires heavy containers) or even Vaultwarden, RustVault is written in pure Rust. The entire backend consumes **less than 15MB of RAM**. You can host it on a 10-year-old Raspberry Pi or the cheapest free-tier VPS in the world without it breaking a sweat.
...
RustVault employs a strict **Zero-Knowledge Architecture**. The server is completely blind. It acts purely as a dumb storage unit for an impenetrable block of encrypted data.
...
RustVault is designed to be trivial to host. It is fully Dockerized.
...
```bash
cd backend
docker build -t rustvault .
docker run -d -p 3000:3000 -v $(pwd)/data:/app/data --name the_monolith rustvault
```
